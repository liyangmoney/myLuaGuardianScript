...
local function generateScript(pkgName)
 ...
 table.insert(sh, " log \"[TEST] 等待超时(60s)，继续执行\"")
 table.insert(sh, " break")
 table.insert(sh, " fi")
 ...
 table.insert(sh, " STABLE_COUNT=0")
 table.insert(sh, " log \"[TEST] 检测到文件变化，当前文件数: $FILES_COUNT\"")
 table.insert(sh, " fi")
 table.insert(sh, " if [ \"$STABLE_COUNT\" -ge 3 ] && [ \"$FILES_COUNT\" -gt 0 ]; then")
 table.insert(sh, " log \"[TEST] 应用初始化完成，文件数: $FILES_COUNT，等待时间: ${ELAPSED}s\"")
 table.insert(sh, " break")
 table.insert(sh, " fi")
 table.insert(sh, " sleep 2")
