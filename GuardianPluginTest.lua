...
local function generateScript(pkgName)
 ...
 table.insert(sh, " log \"[TEST] script.uip未变化，继续等待...\"")
 table.insert(sh, " sleep 2")
 table.insert(sh, " continue")
 table.insert(sh, " fi")
 table.insert(sh, " if [ \"$CURRENT_MTIME\" = \"$BASE_MTIME\" ]; then")
 ...
 table.insert(sh, " else")
 table.insert(sh, " BASE_MTIME=$CURRENT_MTIME")
 table.insert(sh, " STABLE_COUNT=0")
 table.insert(sh, " log \"[TEST] 检测到script.uip变化，新mtime: $CURRENT_MTIME\"")
 table.insert(sh, " fi")
 table.insert(sh, " if [ \"$STABLE_COUNT\" -ge 3 ]; then")
 table.insert(sh, " log \"[TEST] 应用初始化完成，script.uip稳定，总等待时间: ${ELAPSED}s\"")
 table.insert(sh, " break")
 table.insert(sh, " fi")
 table.insert(sh, " sleep 2")
 ...
 table.insert(sh, " log \"[TEST] 备份配置已恢复\"")
 table.insert(sh, " sleep 1")
 table.insert(sh, " RESTART_CNT=0")
 table.insert(sh, " echo 0 > \"$HB_FILE\"")
 table.insert(sh, " LAST_HB=0")
