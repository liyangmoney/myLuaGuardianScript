...
local function generateScript(pkgName)
 ...
 table.insert(sh, " log \"[TEST] 检测到script.uip变化，再等6秒稳定...\"")
 table.insert(sh, " sleep 6")
 table.insert(sh, " log \"[TEST] 应用初始化完成\"")
 table.insert(sh, "")
 table.insert(sh, " log \"[TEST] 恢复备份配置...\"")
 ...
 table.insert(sh, " log "[TEST] 首次启动，超时设为20秒"")
 table.insert(sh, " echo 0 > \"$HB_FILE\"")
 table.insert(sh, " LAST_HB=0")
 table.insert(sh, " START_TIME=$(date +%s)")
 ...
 table.insert(sh, " FIRST_START=1")
 table.insert(sh, " TIMEOUT=20")
 table.insert(sh, " log "[TEST] 重启后首次启动，超时设为20秒"")
 table.insert(sh, " fi")
