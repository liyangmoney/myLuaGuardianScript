...
local function generateScript(pkgName)
 ...
 table.insert(sh, " # 修改时间稳定，计数+1")
 table.insert(sh, " STABLE_COUNT=$((STABLE_COUNT + 1))")
 table.insert(sh, " else")
 table.insert(sh, " # 文件有变化或刚创建，重置稳定计数")
 table.insert(sh, " if [ \"$CURRENT_MTIME\" != \"0\" ]; then")
 table.insert(sh, " LAST_MTIME=$CURRENT_MTIME")
 table.insert(sh, " STABLE_COUNT=0")
 table.insert(sh, " log \"检测到 script.uip 变化，mtime: $CURRENT_MTIME\"")
 table.insert(sh, " fi")
 table.insert(sh, " fi")
 table.insert(sh, " # 连续3次检查稳定（约6秒无变化），认为初始化完成")
 table.insert(sh, " if [ \"$STABLE_COUNT\" -ge 3 ] && [ \"$LAST_MTIME\" != \"0\" ]; then")
 table.insert(sh, " log \"应用初始化完成，script.uip 稳定，等待时间: ${ELAPSED}s\"")
 table.insert(sh, " break")
 table.insert(sh, " fi")
 table.insert(sh, " sleep 2")
 table.insert(sh, " done")
 table.insert(sh, "")
 table.insert(sh, " # 5.5 恢复备份的配置文件")
 table.insert(sh, " BACKUP_CFG=\"/data/data/script.cfg\"")
 table.insert(sh, " TARGET_CFG=\"/data/data/\\\$PKG/files/script.cfg\"")
 table.insert(sh, " if [ -f \"\\\$BACKUP_CFG\" ]; then")
 table.insert(sh, " log \"检测到备份配置，开始恢复...\"")
 table.insert(sh, " if su -c \"cp '\\\$BACKUP_CFG' '\\\$TARGET_CFG'\" 2>/dev/null; then")
 table.insert(sh, " su -c \"chmod 777 '\\\$TARGET_CFG'\" 2>/dev/null || true")
 table.insert(sh, " log \"备份配置已恢复: script.cfg\"")
 table.insert(sh, " elif cp \"\\\$BACKUP_CFG\" \"\\\$TARGET_CFG\" 2>/dev/null; then")
 table.insert(sh, " chmod 777 \"\\\$TARGET_CFG\" 2>/dev/null || true")
 table.insert(sh, " log \"备份配置已恢复: script.cfg\"")
 table.insert(sh, " else")
 table.insert(sh, " log \"警告: 无法恢复备份配置\"")
 table.insert(sh, " fi")
 table.insert(sh, " else")
 table.insert(sh, " log \"未找到备份配置: /data/data/script.cfg\"")
 table.insert(sh, " fi")
 table.insert(sh, " sleep 1")
 table.insert(sh, "")
