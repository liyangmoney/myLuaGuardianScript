...
local function generateScript(pkgName)
 ...
 table.insert(sh, "")
 table.insert(sh, " # 动态等待：检测应用是否完成初始化（最多60秒）")
 table.insert(sh, " log \"等待应用完成初始化...\"")
 table.insert(sh, " WAIT_START=$(date +%s)")
 table.insert(sh, " FILES_COUNT=0")
 table.insert(sh, " STABLE_COUNT=0")
 table.insert(sh, " while true; do")
 table.insert(sh, " NOW=$(date +%s)")
 table.insert(sh, " ELAPSED=$((NOW - WAIT_START))")
 table.insert(sh, " # 检查是否超时（60秒）")
 table.insert(sh, " if [ \"$ELAPSED\" -gt 60 ]; then")
 table.insert(sh, " log \"等待超时(60s)，继续执行\"")
 table.insert(sh, " break")
 table.insert(sh, " fi")
 table.insert(sh, " # 检查文件目录是否有变化（说明应用在创建文件）")
 table.insert(sh, " CURRENT_COUNT=$(su -c \"ls /data/data/\\\$PKG/files/ 2>/dev/null | wc -l\" 2>/dev/null || ls /data/data/\\\$PKG/files/ 2>/dev/null | wc -l)")
 table.insert(sh, " if [ \"$CURRENT_COUNT\" -eq \"$FILES_COUNT\" ]; then")
 table.insert(sh, " # 文件数量稳定，计数+1")
 table.insert(sh, " STABLE_COUNT=$((STABLE_COUNT + 1))")
 table.insert(sh, " else")
 table.insert(sh, " # 文件数量变化，重置稳定计数")
 table.insert(sh, " FILES_COUNT=$CURRENT_COUNT")
 table.insert(sh, " STABLE_COUNT=0")
 table.insert(sh, " log \"检测到文件变化，当前文件数: $FILES_COUNT\"")
 table.insert(sh, " fi")
 table.insert(sh, " # 连续3次检查稳定（约6秒无变化），认为初始化完成")
 table.insert(sh, " if [ \"$STABLE_COUNT\" -ge 3 ] && [ \"$FILES_COUNT\" -gt 0 ]; then")
 table.insert(sh, " log \"应用初始化完成，文件数: $FILES_COUNT，等待时间: ${ELAPSED}s\"")
 table.insert(sh, " break")
 table.insert(sh, " fi")
 table.insert(sh, " sleep 2")
