...
local function generateScript(pkgName)
 ...
 table.insert(sh, " # 浮窗权限需要特殊处理，尝试多种方式")
 table.insert(sh, " if su -c \"appops set '$PKG' SYSTEM_ALERT_WINDOW allow\" 2>/dev/null; then")
 table.insert(sh, " log \"使用su授予浮窗权限成功\"")
 table.insert(sh, " elif appops set \"$PKG\" SYSTEM_ALERT_WINDOW allow 2>/dev/null; then")
 table.insert(sh, " log \"直接授予浮窗权限成功\"")
 table.insert(sh, " fi")
 table.insert(sh, " # 尝试通过settings写入（部分系统有效）")
